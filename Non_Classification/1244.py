# 1244 / calculate final state of switch
N = int(input())
switch = [2] + list(map(int, input().split()))

M = int(input())
for i in range(M):
    g, n = map(int, input().split())  # gender, target number
    if g == 1:  # if gender is man
        j = n
        while j <= N:  # change state of switch which number is multiple of n
            switch[j] = 1 - switch[j]
            j += n
...
while res:  # print result, cutting 40 letters
    print(res[:40])
    res = res[40:]
